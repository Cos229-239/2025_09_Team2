...
Future<void> sendMessage(String content) async {
  ...
  _log('sendMessage called with: "$content"',
  ...
  try {
    ...
    _log('Added user message to list. Total messages: ${_messages.length}',
        level: LogLevel.debug, context: 'sendMessage');
    notifyListeners();
...
Future<void> sendMessage(String content) async {
  ...
  try {
    ...
    _log(
        'Pre-processing complete. Learning style: ${preContext.detectedStyle?.getSummary()}',
        level: LogLevel.info,
        context: 'sendMessage');
    ...
    _log(
        'Post-processing complete. Memory valid: ${postResult.memoryValid}, Math valid: ${postResult.mathValid}',
        level: LogLevel.info,
        context: 'sendMessage');
...
Future<String> _generateAIResponseWithPrompt(
    String content, QueryAnalysis analysis,
    {required String systemPrompt}) async {
  try {
    // Build prompt with enhanced system context
    final enhancedPrompt =
        _buildEnhancedPrompt(content, analysis, systemPrompt);
    ...
    _log('Error in _generateAIResponseWithPrompt: $e',
        level: LogLevel.error, context: '_generateAIResponseWithPrompt');
    rethrow;
  }
}
