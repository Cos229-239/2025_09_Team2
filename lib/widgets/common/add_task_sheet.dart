...
class _AddTaskSheetState extends State<AddTaskSheet> {
  ...
  final _minutesController = TextEditingController(text: '30');        // Estimated duration (default 30 min)
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Expanded(
                      child: TextFormField(
                        controller: _minutesController,  // Controller for duration input
                        keyboardType: TextInputType.number,  // Numeric keyboard
                        decoration: const InputDecoration(
                          labelText: 'Estimated Minutes',
                          border: OutlineInputBorder(),
                        ),
                        validator: (value) {         // Input validation for duration
                          if (value == null || value.isEmpty) {
                        ...
                        child: InputDecorator(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              Text(
                                _selectedDate != null
                                    ? DateFormat('MMM d').format(_selectedDate!)  // Format date (e.g., "Jan 5")
                ...
                SegmentedButton<int>(
                  segments: const [
                    ButtonSegment(value: 1, label: Text('Low')),      // Low priority option
                    ButtonSegment(value: 2, label: Text('Medium')),   // Medium priority option
                    ButtonSegment(value: 3, label: Text('High')),     // High priority option
                  ],
                  selected: {_priority},             // Currently selected priority
                  onSelectionChanged: (Set<int> selected) {
                    setState(() {
                      _priority = selected.first;   // Update priority when selection changes
                    });
                  },
                ),
                const SizedBox(height: 16),         // Spacing between sections
                  ...
                  children: [
                    Expanded(
                      child: TextField(
                        ...
                        onSubmitted: (_) => _addTag(),  // Add tag when Enter pressed
                      ),
                    ),
                  ],
                ),
                ...
                if (_tags.isNotEmpty) ...[
                  ...
                  Wrap(
                    ...
                    children: _tags.map((tag) => Chip(
                      ...
                      onDeleted: () {                // Delete callback for tag removal
                        setState(() {
  ...
  void _selectDate() async {
    final picked = await showDatePicker(
      context: context,
      initialDate: _selectedDate ?? DateTime.now(),        // Default to selected date or today
      firstDate: DateTime.now(),                           // Can't select past dates
      lastDate: DateTime.now().add(const Duration(days: 365)),  // Up to 1 year in future
    );
