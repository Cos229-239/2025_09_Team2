...
class _PersonalizedAIDemoState extends State<PersonalizedAIDemo> {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
                '  • Learning Style: ${user.preferences.learningStyle.toUpperCase()}'),
            Text('  • Difficulty: ${user.preferences.difficultyPreference}'),
            Text('  • Major: ${user.major} at ${user.school}'),
            Text(
                '  • Study Time: ${user.preferences.studyStartHour}:00-${user.preferences.studyEndHour}:00'),
            SizedBox(height: 8),
            ...
            Text(
                '  • CS Performance: ${(analytics.subjectPerformance['Computer Science']!.accuracy * 100).round()}%'),
            Text(
                '  • Trend: ${analytics.recentTrend.direction} (+${analytics.recentTrend.changeRate}%/week)'),
            SizedBox(height: 8),

            // AI Adaptations
            Text('🤖 AI Adaptations Applied:',
                style: TextStyle(fontWeight: FontWeight.bold)),
            Text(
                '  • Visual learner optimization (diagrams, spatial concepts)'),
            Text(
                '  • CS major context (programming examples, technical terms)'),
            Text(
                '  • Adaptive difficulty (currently: ${analytics.getRecommendedDifficulty('Computer Science')})'),
            Text(
                '  • Performance-based adjustments (82% accuracy = moderate challenge)'),
    ...
    return Center(
      child: ElevatedButton(
        onPressed: _isGenerating
            ? null
            : () => _generatePersonalizedCards(_sampleUser, _sampleAnalytics),
        ...
        child: _isGenerating
            ? Row(
                ...
                children: [
                  ...
                  SizedBox(width: 8),
                  Text('Generating Personalized Cards...',
                      style: TextStyle(color: Colors.white)),
                ],
              )
            : Text('🤖 Generate Personalized AI Flashcards',
  ...
  Widget _buildGeneratedCards() {
    if (_generatedCards.isEmpty) {
      return Text(
          'No cards generated yet. Click the button above to create personalized flashcards!');
    }
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                Chip(
                                  label:
                                      Text('Difficulty: ${card.difficulty}/5'),
                                  backgroundColor:
                                ...
                                Chip(
                                  label: Text(
                                      'Correct: ${String.fromCharCode(65 + card.correctAnswerIndex)}'),
  ...
  Future<void> _generatePersonalizedCards(
      User user, StudyAnalytics analytics) async {
    ...
    try {
      ...
      String personalizationInfo =
          'Generated for ${user.preferences.learningStyle} learner (${analytics.performanceLevel} level)';

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text('✅ Personalized cards generated! $personalizationInfo'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text('📝 Demo cards generated (AI service not configured)'),
  ...
  List<FlashCard> _createDemoCards() {
    return [
      FlashCard(
        ...
        type: CardType.basic,
        front:
            'In object-oriented programming, what is the concept that allows a class to inherit properties and methods from another class?',
        back:
      ...
      FlashCard(
        ...
        type: CardType.basic,
        front:
            'Which principle of OOP involves bundling data and methods that operate on that data within a single unit?',
        back:
      ...
      FlashCard(
        ...
        type: CardType.basic,
        front:
            'What OOP concept allows objects of different classes to be treated as objects of a common base class?',
        back:
