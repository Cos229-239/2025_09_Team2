...
class NotificationProvider with ChangeNotifier {
  ...
  int get unreadCount =>
      _notifications.where((n) => !n.isRead && n.shouldDisplay).length;
  int get urgentCount => _notifications
      .where((n) => n.isUrgent && !n.isRead && n.shouldDisplay)
      .length;
  ...
  Future<void> checkQuizNotifications({
    ...
    try {
      final newNotifications =
          await _notificationService.generateQuizNotifications(
        quests: quests,
        dueCards: dueCards,
      );
  ...
  Future<void> _removeExpiredNotifications() async {
    ...
    if (_notifications.length < originalCount) {
      ...
      debugPrint(
          'Removed ${originalCount - _notifications.length} expired notifications');
    }
  }
  ...
  Map<String, int> getNotificationStats() {
    ...
    for (final type in NotificationType.values) {
      final count =
          _notifications.where((n) => n.type == type && n.shouldDisplay).length;
