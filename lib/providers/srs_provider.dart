...
class SRSProvider extends ChangeNotifier {
  ...
  void recordReview(FlashCard card, ReviewGrade grade) {
    ...
    if (existingReviewIndex != -1) {              // If review already exists
      // Update existing review with new grade using SM-2 algorithm
      final existingReview = _reviews[existingReviewIndex];
      updatedReview = existingReview.updateWithGrade(grade);  // Calculate new intervals
      _reviews[existingReviewIndex] = updatedReview;          // Replace with updated review
    } else {                                      // If this is a new card
      // Create new review with initial SM-2 parameters
      updatedReview = Review(
        cardId: card.id,                          // Link to the reviewed card
        userId: 'user123',                        // Placeholder user ID (will be replaced with auth system)
  ...
  DateTime _getInitialDueDate(ReviewGrade grade) {
    switch (grade) {
      case ReviewGrade.again:                     // Complete failure
        return DateTime.now().add(const Duration(minutes: 10));  // Review again in 10 minutes
      case ReviewGrade.hard:                      // Difficult recall
        return DateTime.now().add(const Duration(days: 1));      // Review again tomorrow
  ...
  Map<String, int> getReviewStats() {
    ...
    final todayReviews = _reviews.where((r) => 
    ...
    return {
      ...
      'learning': _reviews.where((r) => r.interval < 7).length,    // Cards in learning phase (< 1 week)
      'mature': _reviews.where((r) => r.interval >= 21).length,    // Mature cards (≥ 3 weeks)
    };
  }
}
