...
class Task {
  ...
  Map<String, dynamic> toJson() => {
        ...
        'dueAt': dueAt
            ?.toIso8601String(), // Convert DateTime to ISO string (null if no deadline)
        ...
        'linkedNoteId':
            linkedNoteId, // Store linked note ID (null if not linked)
        'linkedDeckId':
            linkedDeckId, // Store linked deck ID (null if not linked)
      };
  ...
  factory Task.fromJson(Map<String, dynamic> json) => Task(
        ...
        estMinutes:
            json['estMinutes'] as int, // Extract time estimate as integer
        dueAt: json['dueAt'] != null // Check if deadline exists
            ? DateTime.parse(json['dueAt']
                as String) // Parse ISO string to DateTime if present
            : null, // Set to null if no deadline
        priority: (json['priority'] as int?) ??
            1, // Extract priority, default to 1 if missing
        tags: List<String>.from(json['tags'] ??
            []), // Convert tags to List<String>, default to empty if missing
        status: TaskStatus.values.firstWhere(
          // Find matching enum value
          (e) =>
              e.toString() == json['status'], // Compare string representations
          orElse: () =>
              TaskStatus.pending, // Default to pending if no match found
        ),
        linkedNoteId:
            json['linkedNoteId'] as String?, // Extract note ID (can be null)
        linkedDeckId:
            json['linkedDeckId'] as String?, // Extract deck ID (can be null)
      );
  ...
  Task copyWith({
    ...
    return Task(
      ...
      estMinutes:
          estMinutes ?? this.estMinutes, // Use new estimate or keep current
      ...
      linkedNoteId: linkedNoteId ??
          this.linkedNoteId, // Use new note link or keep current
      linkedDeckId: linkedDeckId ??
          this.linkedDeckId, // Use new deck link or keep current
    );
  }
}
