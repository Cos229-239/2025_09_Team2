...
class Review {
  ...
  Review updateWithGrade(ReviewGrade grade) {
    ...
    switch (grade) {
      case ReviewGrade.again:        // User forgot the card completely
        ...
        newEase = ease - 0.2;        // Decrease ease factor (make future intervals shorter)
        break;
      ...
      case ReviewGrade.good:         // User remembered correctly
        if (reps == 0) {             // First time reviewing this card
          newInterval = 1;           // Keep at 1 day for first review
        } else if (reps == 1) {      // Second time reviewing
          newInterval = 6;           // Jump to 6 days for second review
        } else {                     // Subsequent reviews
          newInterval = (interval * ease).round();  // Use ease factor for interval
        }
        break;
        
      case ReviewGrade.easy:         // User remembered very easily
        newInterval = (interval * ease * 1.3).round();  // Longer interval increase
        newEase = ease + 0.15;       // Increase ease factor (make future intervals longer)
        break;
    }
    ...
    return Review(
      ...
      dueAt: DateTime.now().add(Duration(days: newInterval)),  // Calculate new due date
  ...
  Map<String, dynamic> toJson() => {
    ...
    'lastReviewed': lastReviewed?.toIso8601String(),     // Store review time as ISO string (null safe)
  };
  ...
  factory Review.fromJson(Map<String, dynamic> json) => Review(
    ...
    dueAt: DateTime.parse(json['dueAt'] as String),      // Parse due date from ISO string
    ...
    lastGrade: json['lastGrade'] != null 
      ? ReviewGrade.values.firstWhere(
          (e) => e.toString() == json['lastGrade'],      // Find matching enum value
    ...
    lastReviewed: json['lastReviewed'] != null 
      ? DateTime.parse(json['lastReviewed'] as String)   // Parse ISO string to DateTime
