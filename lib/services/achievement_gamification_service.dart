...
class AchievementGamificationService {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      if (user == null) {
        debugPrint(
            'AchievementService: No authenticated user, loading local data only');
  ...
  Future<void> _initializeFirebaseData(String userId) async {
    try {
      ...
      final userDoc =
          await _firestore.collection(_userDataCollection).doc(userId).get();
      ...
      if (userDoc.exists) {
        ...
        if (data['level'] != null) {
          _userLevel =
              UserLevel.fromJson(Map<String, dynamic>.from(data['level']));
        }
  ...
  Future<void> _loadAchievementDefinitions() async {
    try {
      ...
      if (cloudAchievements.isNotEmpty) {
        ...
        debugPrint(
            'AchievementService: Loaded ${_achievements.length} achievements from Firestore');
      }
    } catch (e) {
      debugPrint(
          'AchievementService: Error loading achievement definitions: $e');
    }
  }
  ...
  Future<void> _setupRealtimeListeners(String userId) async {
    ...
    _seasonalEventsSubscription = _firestore
      ...
      debugPrint(
          'AchievementService: ${_activeSeasonalEvents.length} active seasonal events');
    });
  }
  ...
  Future<void> _loadSeasonalEvents() async {
    try {
      if (!_isOnline) {
        debugPrint(
            'AchievementService: Offline, skipping seasonal events load');
        return;
      }
      ...
      debugPrint(
          'AchievementService: Loaded ${_activeSeasonalEvents.length} seasonal events');
    } catch (e) {
  ...
  Future<List<Achievement>> _checkAchievements(
    ...
    for (final achievement in _achievements.values) {
      ...
      if (!_validateAchievementProgress(
          achievement.id, newProgress, sessionData)) {
        debugPrint(
            'AchievementService: Invalid progress detected for ${achievement.id}');
        continue;
      }
  ...
  Future<bool> shareAchievement(Achievement achievement) async {
    ...
    try {
      ...
      await _firestore.collection(_achievementSharesCollection).add(shareData);
    ...
    try {
      ...
      for (final doc in snapshot.docs) {
        ...
        leaderboard.add({
          ...
          'achievements_count': (data['progress'] as Map?)
                  ?.values
  ...
  Future<void> generateDynamicAchievements(
      Map<String, dynamic> userBehavior) async {
    if (!_isOnline || _auth.currentUser == null) {
      return;
    }
    ...
    try {
      ...
      if (studyHour != null) {
        final timeLabel = studyHour < 12
            ? 'Morning'
            : (studyHour < 18 ? 'Afternoon' : 'Evening');
        dynamicAchievements.add(Achievement(
      ...
      debugPrint(
          'AchievementService: Generated ${dynamicAchievements.length} dynamic achievements');
    } catch (e) {
      debugPrint(
          'AchievementService: Error generating dynamic achievements: $e');
    }
  }
  ...
  bool _validateAchievementProgress(
    ...
    if (recentProgress > 10) {
      ...
      debugPrint(
          'AchievementService: Suspicious rapid progress detected for $achievementId');
  ...
  Future<bool> importUserData(Map<String, dynamic> data) async {
    try {
      ...
      if (data['user_level'] != null) {
        _userLevel =
            UserLevel.fromJson(Map<String, dynamic>.from(data['user_level']));
      }
  ...
  List<SeasonalEvent> get activeSeasonalEvents =>
      List.from(_activeSeasonalEvents);
  ...
  Future<void> forceSync() async {
    if (!_isOnline || _auth.currentUser == null) {
      debugPrint(
          'AchievementService: Cannot sync while offline or unauthenticated');
      return;
    }
