...
class QuizService {
  ...
  FlashCard recordQuizAttempt({
    ...
    debugPrint(
        'Quiz attempt recorded for card ${card.id}: ${correct ? "CORRECT" : "INCORRECT"}');
  ...
  Deck getDeckWithAttempts(Deck deck) {
    final updatedCards =
        deck.cards.map((card) => getCardWithAttempts(card)).toList();
  ...
  Map<String, dynamic> getQuizStats() {
    ...
    final correctAttempts =
        attempts.where((card) => card.lastQuizCorrect == true).length;
    final successRate = correctAttempts / totalAttempts;
    final averageDifficulty =
        attempts.map((card) => card.difficulty).reduce((a, b) => a + b) /
            totalAttempts;
