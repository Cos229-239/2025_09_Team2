...
class WebSearchService {
  ...
  void initialize() {
    ...
    if (!GeminiConfig.enableWebSearch) {
      developer.log('Web search is disabled in config',
    ...
    try {
      ...
      debugPrint(
          '🌐 DEBUG: GenerativeModel created! _model != null? ${_model != null}');
      developer.log(
          '✅ Gemini model initialized successfully! Model is ${_model != null ? "available" : "NULL"}',
          name: 'WebSearchService');
    } catch (e) {
      debugPrint('🌐 DEBUG: EXCEPTION during model creation: $e');
      developer.log('❌ Failed to initialize Gemini: $e',
          name: 'WebSearchService');
    }
  }
  ...
  Future<SearchResult> search(
    ...
    if (!isAvailable) {
      debugPrint(
          '🌐 DEBUG: search() - NOT AVAILABLE! _model=${_model != null}, enableWebSearch=${GeminiConfig.enableWebSearch}');
      return SearchResult(
        answer:
            'Web search is not available. Please check your API configuration.',
        timestamp: startTime,
        query: query,
    ...
    try {
      ...
      if (_cache.containsKey(cacheKey)) {
        ...
        if (age.inMinutes < 5) {
          ...
          developer.log('Returning cached result for: $query',
              name: 'WebSearchService');
          return SearchResult(
      ...
      debugPrint(
          '🌐 DEBUG: Calling Gemini API with prompt length=${prompt.length}');
      developer.log('Performing web search for: $query',
          name: 'WebSearchService');

      // Perform search with timeout
      final response = await _model!.generateContent(
          [Content.text(prompt)]).timeout(GeminiConfig.searchTimeout);
  ...
  String _buildSearchPrompt(String query, String? context) {
    ...
    buffer.writeln(
        'You are an expert research assistant with access to current web information.');
    buffer.writeln(
        'Your role is to provide accurate, well-researched answers to student questions.');
    buffer.writeln();
    ...
    buffer.writeln(
        '1. Search for current, accurate information about this query');
    buffer.writeln('2. Provide a clear, concise answer suitable for a student');
    buffer.writeln(
        '3. If you find specific facts (names, dates, numbers), include them');
    buffer.writeln(
        '4. If information might be outdated or uncertain, mention when it was last verified');
    buffer.writeln('5. Focus on educational value and accuracy');
    buffer.writeln(
        '6. Keep the response under 300 words unless more detail is specifically needed');
    buffer.writeln();
