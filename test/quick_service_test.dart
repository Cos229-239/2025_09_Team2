...
void main() {
  group('Math Engine Tests', () {
    test('Validates correct math', () async {
      final result =
          await MathEngine.validateAndAnnotate('The answer is 2 + 2 = 4');
      expect(result.valid, isTrue, reason: 'Correct math should be valid');
    ...
    test('Catches simple math errors', () async {
      final result =
          await MathEngine.validateAndAnnotate('The answer is 2 + 2 = 5');
      expect(result.valid, isFalse, reason: 'Should detect 2+2=5 is wrong');
    ...
    test('Solves and shows steps', () async {
      ...
      expect(result, isNotEmpty, reason: 'Should return solution steps');
      // Check if any step contains the answer 4
      final hasCorrectAnswer = result.any((step) =>
          step.toString().contains('4') ||
          step.result.toString().contains('4'));
  ...
  group('Learning Style Detector Tests', () {
    test('Detects visual preferences from keywords', () {
      ...
      session.addMessage(ChatMessage(
        id: '1',
        content:
            'Can you show me a diagram? I need visual examples and pictures.',
      ...
      expect(style.preferences.visual > style.preferences.auditory, isTrue,
          reason:
              'Message with "diagram", "visual", "pictures" should score high on visual');
    ...
    test('Detects auditory preferences', () {
      ...
      session.addMessage(ChatMessage(
        id: '1',
        content:
            'Can you explain it verbally? I learn by listening and hearing explanations.',
        type: MessageType.user,
        format: MessageFormat.text,
        timestamp: DateTime.now(),
      ));
      ...
      expect(style.preferences.auditory > style.preferences.visual, isTrue,
          reason:
              'Message with "verbally", "listening", "hearing" should score high on auditory');
    });
    ...
    test('Detects kinesthetic preferences', () {
      ...
      expect(style.preferences.kinesthetic > 0.0, isTrue,
          reason:
              'Message with "hands-on", "practice", "do" should score on kinesthetic');
    });
    ...
    test('Detects reading/writing preferences', () {
      ...
      session.addMessage(ChatMessage(
        id: '1',
        content:
            'Can you write it down? I like taking notes and reading documentation.',
        type: MessageType.user,
        format: MessageFormat.text,
        timestamp: DateTime.now(),
      ));
      ...
      expect(style.preferences.reading > 0.0, isTrue,
          reason:
              'Message with "write", "notes", "reading" should score on reading/writing');
    });
  ...
  group('Memory Claim Validator Tests', () {
    ...
    test('Validates legitimate recall with session history', () {
      ...
      expect(result.valid, isTrue,
          reason:
              'Should validate legitimate recall when topic was actually discussed');
    });

    test('Generates honest alternative for false claims', () {
      final alternative =
          MemoryClaimValidator.generateHonestAlternative('algebra');
  ...
  group('SessionContext Tests', () {
    ...
    test('Extracts and tracks topics', () {
      ...
      final topicsDetected = topics.isNotEmpty ||
          hasPhotosynthesis ||
          hasCellular ||
          hasRespiration;
      ...
      expect(topicsDetected, isTrue,
          reason:
              'Should detect topics either in getRecentTopics or hasDiscussedTopic');
    });
    ...
    test('Returns recent messages correctly', () {
      ...
      expect(recent.length, equals(3), reason: 'Should return last 3 messages');
